...
class CheckoutController extends Controller
{
    ...
    public function store(Request $request, PricingService $pricing, ZarinpalGateway $gateway): RedirectResponse
    {
        $validated = $request->validate([
            'customer_name'=>['required','string','max:120'],'phone'=>['required','string','max:20'],'email'=>['nullable','email','max:190'],
            'province'=>['required','string','max:80'],'city'=>['required','string','max:80'],'address'=>['required','string','max:500'],'postal_code'=>['nullable','string','max:20'],'notes'=>['nullable','string','max:1000'],
        ]);
        $summary = $this->summary($request, $pricing);
        ...
        try {
            $order = DB::transaction(function () use ($validated, $summary): Order {
                ...
                foreach ($summary['items'] as $item) {
                    ...
                    $order->items()->create(['product_id'=>$product->id,'product_name'=>$product->name,'sku'=>$product->sku,'quantity'=>$item['quantity'],'unit_price'=>$item['unitPrice'],'total_price'=>$item['total'],'meta'=>['width'=>$product->width,'height'=>$product->height]]);
                }
                return $order;
            });
    ...
    private function summary(Request $request, PricingService $pricing): array
    {
        ...
        $items = $cart->map(function (array $row, int|string $productId) use ($products,$pricing) {
            $product=$products->get((int)$productId); if(!$product||!$product->is_active||$product->stock<1)return null;
            $quantity=max(1,min((int)$row['quantity'],$product->stock)); $unitPrice=$pricing->productPrice($product);
            return compact('product','quantity','unitPrice')+['total'=>$unitPrice*$quantity];
        })->filter()->values();
        $subtotal=(int)$items->sum('total'); $couponCode=$request->session()->get('coupon_code'); $coupon=$pricing->coupon($couponCode,$subtotal);
        $shipping=(int)Setting::valueOf('shipping_flat',config('store.shipping_flat',0)); $discount=(int)$coupon['discount'];
        return compact('items','subtotal','discount','shipping','couponCode')+['total'=>max(0,$subtotal-$discount+$shipping)];
    }
}
