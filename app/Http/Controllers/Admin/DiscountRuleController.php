...
class DiscountRuleController extends Controller
{
    ...
    private function payload(Request $request):array { $data=$request->validate(['name'=>['required','string','max:190'],'type'=>['required',Rule::in(['percent','fixed'])],'scope'=>['required',Rule::in(['all','product','category'])],'scope_value'=>['nullable','array'],'scope_value.*'=>['integer'],'value'=>['required','integer','min:1'],'priority'=>['nullable','integer'],'starts_at'=>['nullable','date'],'ends_at'=>['nullable','date','after_or_equal:starts_at']]); return $data+['scope_value'=>$data['scope']==='all'?[]:array_values($data['scope_value']??[]),'priority'=>(int)($data['priority']??0),'is_active'=>$request->boolean('is_active',true)]; }
