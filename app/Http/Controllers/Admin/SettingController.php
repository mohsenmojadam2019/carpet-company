...
class SettingController extends Controller
{
    public function edit(): View
    {
        ...
        $keys = ['store_name','store_tagline','store_phone','store_address','instagram_url','seo_default_title','seo_default_description','shipping_flat','zarinpal_sandbox','kavenegar_sender','kavenegar_verify_template'];
        $settings = collect($keys)->mapWithKeys(fn (string $key) => [$key => Setting::valueOf($key, '')]);
        $status = [
            'zarinpal_configured' => filled(Setting::valueOf('zarinpal_merchant_id', config('services.zarinpal.merchant_id'))),
            'kavenegar_configured' => filled(Setting::valueOf('kavenegar_api_key', config('services.kavenegar.api_key'))),
        ];
    ...
    public function update(Request $request): RedirectResponse
    {
        ...
        $data = $request->validate([
            ...
            'zarinpal_merchant_id'=>['nullable','string','max:100'],'zarinpal_sandbox'=>['nullable','boolean'],'kavenegar_api_key'=>['nullable','string','max:255'],'kavenegar_sender'=>['nullable','string','max:40'],'kavenegar_verify_template'=>['nullable','string','max:100'],
        ]);
