...
class PaymentController extends Controller
{
    public function __invoke(Request $request, ZarinpalGateway $gateway, KavenegarSms $sms): RedirectResponse
    {
        ...
        if($order->payment_status==='paid') return redirect()->route('orders.success',$order);
        ...
        try {
            ...
            $inventoryReview=DB::transaction(function() use($order,$verified,$authority):bool {
                ...
                $newStatus=$inventoryReview?'inventory_review':'processing';
                $from=$locked->status;
                $locked->update([
                    'status'=>$newStatus,'payment_status'=>'paid','payment_authority'=>$authority,'payment_ref_id'=>$verified['ref_id'],'paid_at'=>now(),
                    'invoice_number'=>'INV-'.now()->format('ymd').'-'.str_pad((string)$locked->id,6,'0',STR_PAD_LEFT),
                ]);
                $locked->statusHistories()->create([
                    'from_status'=>$from,'to_status'=>$newStatus,'note'=>$inventoryReview?'پرداخت تأیید شد؛ سفارش نیازمند بررسی موجودی است.':'پرداخت زرین‌پال تأیید شد و سفارش وارد پردازش شد.',
                    'meta'=>['payment_ref_id'=>$verified['ref_id'],'gateway'=>'zarinpal'],
                ]);
            ...
            return redirect()->route('orders.success',$order)->with('success',$inventoryReview?'پرداخت موفق بود؛ سفارش برای بررسی موجودی ثبت شد.':'پرداخت موفق بود و فاکتور شما صادر شد.');
        } catch(Throwable $e){
            report($e);
