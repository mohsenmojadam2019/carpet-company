...
class DatabaseSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $categories = collect([
            ...
            return Category::updateOrCreate(['slug' => $row['slug']], $row + ['sort_order' => $index + 1, 'is_active' => true]);
        })->keyBy('slug');
        ...
        foreach ($products as $index => $row) {
            ...
            Product::updateOrCreate(['sku' => $row['sku']], [
                ...
                'short_description' => 'پالت روشن و خنثی با بافت دقیق؛ مناسب دکوراسیون مدرن، مینیمال و نئوکلاسیک.',
                'description' => 'این محصول با تمرکز بر تناسب رنگ، کیفیت بافت و دوام انتخاب شده است. پیش از خرید می‌توانید برای انتخاب ابعاد و هماهنگی با فضای خانه مشاوره دریافت کنید.',
                'price' => $row['price'], 'sale_price' => $row['sale_price'], 'stock' => $row['stock'],
                'width' => $row['width'], 'height' => $row['height'], 'material' => $row['material'],
                'weave' => $row['weave'], 'density' => $row['density'], 'origin' => $row['origin'],
                'colors' => ['کرم', 'شیری', 'خاکی'], 'images' => ['/images/rug-01.svg'], 'variants' => [],
                'specifications' => ['washable' => true, 'consultation' => true], 'featured' => $index < 4, 'is_active' => true,
                'meta_title' => $row['name'].' | خرید فرش و قالی',
                'meta_description' => 'مشاهده مشخصات، قیمت، ابعاد و خرید '.$row['name'].' با ارسال تخصصی و پرداخت امن.',
            ]);
        }

        Coupon::updateOrCreate(['code' => 'WELCOME10'], [
            'type' => 'percent', 'value' => 10, 'min_order' => 10000000,
            'max_discount' => 5000000, 'usage_limit' => 200, 'used_count' => 0,
            'starts_at' => now()->subDay(), 'ends_at' => now()->addMonths(3), 'is_active' => true,
        ]);

        foreach ([
            ['title' => 'ویلای روشن لواسان', 'slug' => 'lavasan-light-villa', 'location' => 'لواسان', 'excerpt' => 'پالت کرم و خاکی با قالی مرکزی در نشیمن اصلی.'],
            ['title' => 'سوئیت بوتیک تهران', 'slug' => 'tehran-boutique-suite', 'location' => 'تهران', 'excerpt' => 'بافت مینیمال برای فضای جمع‌وجور و نور طبیعی.'],
            ['title' => 'لابی اقامتگاه کویر', 'slug' => 'desert-residence-lobby', 'location' => 'یزد', 'excerpt' => 'ترکیب قالی ایرانی با خطوط معماری معاصر.'],
        ] as $project) {
            Project::updateOrCreate(['slug' => $project['slug']], $project + ['year' => (int) now()->year, 'is_featured' => true, 'is_active' => true]);
        }
    }
}
